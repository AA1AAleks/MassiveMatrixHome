...
public class Matrices {
    ...
    public static class Matrix {
        ...
        public Matrix add(Matrix other) {
            ...
            for (int i = 0; i < nRows; i++) {
                for (int j = 0; j <nCols; j++) {
                    sum.rows[i][j] = A.rows[i][j]+other.rows[i][j];
        ...
        public Matrix subtract(Matrix other) {
            ...
            for (int i = 0; i < nRows; i++) {
                for (int j = 0; j <nCols; j++) {
                    result.rows[i][j] = A.rows[i][j]-other.rows[i][j];
        ...
        public Matrix multiply(Matrix other) {
            Matrix A = this;
            Matrix mul = new Matrix(nRows,nCols);
            for (int i = 0; i < A.nRows; i++) {
                for (int j = 0; j < other.nCols; j++) {
                    mul.rows[i][j]= 0;
                    for (int k = 0; k < A.nCols; k++) {
                        mul.rows[i][j] = A.rows[i][j]*other.rows[i][j];
        ...
        public Matrix scalarMultiply(int scalar) {
            Matrix scalaRes = new Matrix(nRows,nCols);
            for (int i = 0; i < nRows; i++) {
                for (int j = 0; j < nCols; j++) {
                    scalaRes.rows[i][j] = this.rows[i][j] * scalar;
        ...
        public int determinant() {
            ...
            determinant -= rows[0][1] * (rows[1][0] * rows[2][2] - rows[1][2] * rows[2][0]);
            determinant += rows[0][2] * (rows[1][0] * rows[2][1] - rows[1][1] * rows[2][0]);
